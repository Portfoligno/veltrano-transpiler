use veltrano::{BinaryExpr, BinaryOp, CallExpr, Expr, ExprExt, LiteralExpr, MethodCallExpr};
...
fn test_find_subexpressions() {
    ...
    let identifiers = expr.find_subexpressions(|e| matches!(e, Expr::Identifier(_)));
...
fn test_any_subexpr() {
    ...
    let expr = Expr::Binary(BinaryExpr {
        left: Box::new(Expr::Literal(LiteralExpr::Int(42))),
        operator: BinaryOp::Add,
        right: Box::new(Expr::Identifier("x".to_string())),
    });

    // Check if any sub-expression is an integer literal
    assert!(expr.any_subexpr(|e| matches!(e, Expr::Literal(LiteralExpr::Int(_)))));

    // Check if any sub-expression is a string literal
    assert!(!expr.any_subexpr(|e| matches!(e, Expr::Literal(LiteralExpr::String(_)))));
}
...
fn test_all_subexprs() {
    ...
    assert!(expr.all_subexprs(|e| { matches!(e, Expr::Identifier(_) | Expr::Binary(_)) }));

    // Check if all sub-expressions are identifiers (should be false because of Binary)
    assert!(!expr.all_subexprs(|e| matches!(e, Expr::Identifier(_))));
}
        ...
        args: vec![
            ...
            veltrano::Argument::Bare(
                Expr::MethodCall(MethodCallExpr {
                    object: Box::new(Expr::Identifier("c".to_string())),
                    method: "method".to_string(),
                    args: vec![Expr::Identifier("d".to_string())],
                    inline_comment: None,
                    id: 0,
                }),
                None,
