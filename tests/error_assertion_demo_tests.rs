...
fn test_fluent_error_assertions() {
    ...
    let parse_result = parse_veltrano_code("val = 42", Config { preserve_comments: false });
    match parse_result {
...
fn test_double_minus_syntax_error() {
    ...
    let result = parse_veltrano_code("val x = --5", Config { preserve_comments: false });
    match result {
...
fn test_error_with_help_and_note() {
    use veltrano::error::{VeltranoError, SourceLocation, Span};
