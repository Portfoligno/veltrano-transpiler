...
fn test_unterminated_strings() {
    ...
    assert!(try_parse(r#"val x = "unterminated"#).is_ok(), 
...
fn test_unterminated_comments() {
    ...
    assert!(try_parse("/* unterminated comment").is_ok(), 
            "TODO: Unterminated block comments are currently accepted");
    
    assert!(try_parse("/* nested /* comment */ unterminated").is_ok(), 
...
fn test_invalid_escape_sequences() {
    ...
    assert!(try_parse(r#"val x = "invalid \q escape""#).is_ok(), 
...
fn test_invalid_number_literals() {
    // Hex without digits
    assert!(try_parse("val x = 0x").is_err(), 
            "Hex prefix without digits correctly rejected");
    
    // Multiple decimal points
    assert!(try_parse("val x = 1.2.3").is_err(), 
            "Multiple decimal points correctly rejected");
    
    // Trailing decimal point
    assert!(try_parse("val x = 123.").is_err(), 
...
fn test_type_annotations() {
    assert!(try_parse("val x: = 5").is_err(), 
            "Missing type after colon correctly rejected");
    
    assert!(try_parse("val x: Vec<").is_err(), 
...
fn test_control_flow_errors() {
    assert!(try_parse("if { }").is_err(), 
            "If without condition correctly rejected");
    
    assert!(try_parse("if (true)").is_err(), 
            "If without body correctly rejected");
    
    assert!(try_parse("else { }").is_err(), 
