...
fn test_new_style_error_check() {
    ...
    assert_parse_fails_at("val x =", ErrorKind::UnexpectedEof, 1, 8);
}
...
fn test_old_style_multiple_scenarios() {
    use common::{parse_and_type_check, parse_veltrano_code};

    // Test 1: Parse error
    let result1 = parse_veltrano_code(
        "val =",
        Config {
            preserve_comments: false,
        },
    ...
    // Test 2: Type error
    let result2 = parse_and_type_check(
        "val x = unknown",
        Config {
            preserve_comments: false,
        },
    ...
    assert!(result2
        .unwrap_err()
        .to_string()
        .contains("Variable 'unknown' not found"));

    // Test 3: Another parse error
    let result3 = parse_veltrano_code(
        "fun test(",
        Config {
            preserve_comments: false,
        },
...
fn test_new_style_multiple_scenarios() {
    // Test 1: Parse error
    assert_parse_fails("val =", ErrorKind::SyntaxError, "Expected variable name");
    ...
    // Test 3: Another parse error
    assert_parse_fails("fun test(", ErrorKind::SyntaxError, "Expected");
}
